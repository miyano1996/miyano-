const { login,register,isLogin,Adminlogin } = require("../dao/managersDao.js")


module.exports.Adminlogin=async data=>{//管理员登录
    const goLogin=await Adminlogin(data);
    if(goLogin.length){
        //将密码传到第一层用自带方法对比
        return {msg:'登录成功',success:true,password:goLogin[0].password,_id:goLogin[0]._id}
    }else{
