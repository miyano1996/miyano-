<template>
  <main>
    <div class="register">
      ...
      <el-form :model="users" status-icon :rules="rules" ref="ISverify" class="demo-ruleForm">
        <!-- 账号 -->
        ...
        <router-link to="/" tag="p" class="span1">
          <i class="el-icon-s-custom"></i>已有账号去登录>>
        </router-link>
        <router-link to="/registerMg" tag="p" class="span2">
          <i class="el-icon-s-shop"></i>商家注册通道>>
        </router-link>
      </el-form>
      <el-divider content-position="left">
        <i>已阅读并同意蜗牛《用户协议》、《隐私政策》</i>
      </el-divider>
    </div>
  </main>
</template>
    ...
    return {
      ...
      rules: {
        ...
        name: [
          ...
          {
            required: true,
            message: "需要实名认证，请输入真实姓名。",
            trigger: "blur",
          },
          { min: 2, max: 6, message: "长度在 2 到 6 个字符", trigger: "blur" },
        ],
        gender: [{ required: true, message: "请完善性别", trigger: "change" }],
